...
const ContactForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      try {
        // The form will be automatically handled by Netlify Forms
      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 20 }}
            className="fixed top-4 right-4 bg-[#8CEAB3] text-black px-6 py-3 rounded-lg shadow-lg z-50"
          >
            Thank you, your information was submitted successfully! We'll
            contact you soon!
          </motion.div>
        )}
      </AnimatePresence>
